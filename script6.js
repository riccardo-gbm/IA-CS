...
function calcularCosto() {
    ...
    if (vivienda === "r_keele") {
        ...
        servicios = 50;
        alimentacion = 0;
    } else if(vivienda==="r_glendon"){
        renta = 750+((10810.54/8)*12) ;
        servicios = 50;
        alimentacion = 0;
    } else if(vivienda==="r_markham"){
        renta = 750+((1400)*12) ;
        servicios = 50;
        alimentacion = 0;
    }else {
    ...
    let costoTransporte = transporte === "bicicleta" ? 900 + ((taxis * 20 * 4)*12) : (128 + (taxis * 20 * 4))*12;
    ...
    let entretenimiento = (cine * 14) + (discoteca * 4 * 15) + (bebidas * 7) + (suscripciones * 10);
    ...
    document.getElementById("resultado").innerText = `Costo de vida anual en Toronto: ${total.toLocaleString('en-CA', { style: 'currency', currency: 'CAD' })}`;
}